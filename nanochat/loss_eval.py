...
def evaluate_bpb(model, batches, steps, token_bytes, tokenizer=None):
    ...
    for _ in range(steps):
        x, y, x_mods, y_mods = unpack_eval_batch(next(batch_iter))
        if x_mods is None:
            ...
            loss2d = compositional_joint_nll_sum_groups(model, x, y, x_mods, y_mods)
        loss2d = loss2d.view(-1) # flatten
        if y_mods is not None:
            counted = target_counted_mask(y.view(-1), token_bytes)
            total_nats += loss2d[counted].sum()
            num_bytes2d = compositional_target_bytes(y.view_as(loss2d), y_mods, token_bytes, tokenizer)
            total_bytes += num_bytes2d.sum()
        elif (y.int() < 0).any(): # mps does not currently have kernel for < 0 for int64, only int32
            y = y.view(-1) # flatten
            # slightly more complex code path if some target tokens are ignore_index (e.g. -1)
            # any target token < 0 is to be ignored: do NOT index token_bytes with negatives
            counted = target_counted_mask(y, token_bytes)
